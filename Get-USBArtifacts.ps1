<#
  .SYNOPSIS
    Obtains USB artifacts from various registry keys.
  .PARAMETER System
    Attempt to access registry keys which may require system level priviledges.
  .PARAMETER Object
...
function Get-USBArtifacts
{
  ...
  $Block = {
    ...
    if(!$Object)
    {
      foreach($Key in $Table.Keys)
      {
        ...
        {
          Write-Host ($Key + "---" + $Subkey + "---" + $Table.$Key.$Subkey)
        }
      }
    }
  ...
  if($ComputerName -ne "")
  {
    Invoke-Command -ComputerName $ComputerName -ScriptBlock $Block -ArgumentList $Object,$System
  }
  else
  {
    Invoke-Command -ScriptBlock $Block -ArgumentList $Object,$System
  }
}
